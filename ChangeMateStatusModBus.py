...
#ServerPath        = '//192.168.0.100/web/MonitorMate_mod/WebServer' # DPO only for debug 
...
def ErrorPrint (str) :
    try:
        with open(ServerPath + "/data/error.log","r") as file:
            save = file.read()
        with open(ServerPath + "/data/error.log","w") as file:
            file = open(ServerPath + "/data/error.log","a")
            file.write(now.strftime("%d/%m/%Y %H:%M:%S "))
            #file.write(str + "\r\n")
            file.write(str + "\n")
            print(str)
        with open(ServerPath + "/data/error.log","a") as file:
            file.write(save)
        return
    except OSError:
...
if smart_weather == "true":
    logging.info("Weather module active. Checking forceast")
    try:
        ...
        if daily_clouds > clouds_limit:
            ...
            logging.info("... clouds above limits")
            ErrorPrint("Info : CMS - WT: clouds "+ str(daily_clouds) + "% - above limit")
        mycursor.close()
        mydb.close()
...
while True:
    ...
    if Sched_1_check_flag == 0 and Sched_2_check_flag == 0 and FNDC_flag == 0:
        if loop >0:
